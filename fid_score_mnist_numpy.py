...
def calculate_fid_mnist_npy(path, batch_size, cuda, dims, rand_data):
    ...
    m1 = np.array([])
    s1 = np.array([])
    if (os.path.exists('./mnist-mean.npy') and os.path.exists('./mnist-cov.npy')):
        m1 = np.load('./mnist-mean.npy')
        s1 = np.load('./mnist-cov.npy')
    else:
        ...
        act1 = get_activations_numpy(test_set[:1000], model, batch_size, dims, cuda)
    ...
    act2 = get_activations_numpy(np_1[:1000], model, batch_size, dims, cuda)
